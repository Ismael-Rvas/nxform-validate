    ...
    <form action={action} className="my-20 border-2 p-4 flex flex-col gap-4">
      ...
      {state?.issues?.nombre && (
        <div className="text-sm font-medium text-red-600 bg-red-50 rounded-md flex items-center border">
          {<CircleAlert className="inline m-4 mr-2 size-4" />}{" "}
          {state.issues.nombre}
        </div>
      )}
      ...
      <div className="flex justify-between px-4 py-2 rounded-md bg-slate-200">
        ...
        <input
          ...
          className="ring-2"
          defaultValue={
            state.payload?.get("fecha") ||
            new Date().toISOString().split("T")[0]
          } // para recuperar el valor introducido previamente
        //   min="2023-01-01" max="2025-01-13"
          // minimo 1 enero 2023, maximo 13 enero 2025 (fecha actual)
        />
      </div>
      ...
      >
        {pending ? (
          <RefreshCcw className="inline animate-spin size-4" />
        ) : (
          "Insertar"
        )}
      </button>
